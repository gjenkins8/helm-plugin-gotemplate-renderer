...
func ProcessDependencies(c *chart.Chart, v releasevalues.Values) error {
...
func processDependencyConditions(reqs []*chart.Dependency, cvals releasevalues.Values, cpath string) {
	if reqs == nil {
		return
	}
	for _, r := range reqs {
		for _, c := range strings.Split(strings.TrimSpace(r.Condition), ",") {
			if len(c) > 0 {
				...
				} else if _, ok := err.(releasevalues.ErrNoValue); !ok {
...
func processDependencyTags(reqs []*chart.Dependency, cvals releasevalues.Values) {
	if reqs == nil {
		return
	}
...
func pathToMap(path string, data map[string]interface{}) map[string]interface{} {
	...
	return set(releasevalues.ParsePath(path), data)
}
...
func processImportValues(c *chart.Chart, merge bool) error {
	...
	var cvals releasevalues.Values
	var err error
	if merge {
