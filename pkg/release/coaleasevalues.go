...
func CoalesceValues(chrt *chart.Chart, vals map[string]interface{}) (releasevalues.Values, error) {
...
// business logic.
func MergeValues(chrt *chart.Chart, vals map[string]interface{}) (releasevalues.Values, error) {
	valsCopy, err := copyValues(vals)
	if err != nil {
		return vals, err
	}
...
func copyValues(vals map[string]interface{}) (releasevalues.Values, error) {
...
func coalesceDeps(printf printFn, chrt *chart.Chart, dest map[string]interface{}, prefix string, merge bool) (map[string]interface{}, error) {
	for _, subchart := range chrt.Dependencies() {
		...
		} else if !releasevalues.IsTable(c) {
...
func coalesceGlobals(printf printFn, dest, src map[string]interface{}, prefix string, _ bool) {
	...
	for key, val := range sg {
		...
		} else if dv, ok := dg[key]; ok && releasevalues.IsTable(dv) {
